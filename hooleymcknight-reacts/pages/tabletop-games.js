...
import TTGGamesList from '../components/tabletopGames/tabletopGamesList';
import WheelTitle from '../components/wheel/wheelTitle';
import GamesListDesc from '../components/wheel/gamesListDesc';

import gamesDataRaw from '../components/tabletopGames/tabletopGames_data.json';
import Filters from '../components/tabletopGames/ttgFilters';
...
export default function TTGPage() {
  ...
  const applyChange = (type, data, dataToggle, target) => {
    if (type === 'game') { // dataToggle for this is probably reverse of what I would want it. boolean to text is dumb.
      ...
      let relevantFilteredGames = gamesData.filter(x => x[type].includes(data));
  ...
  return (
    <Layout page='tabletop-games-wheel'>
      <Head>
        <title>{`${siteTitle} || Tabletop Games Wheel`}</title>
        <meta name="description" content="Similar to the Jackbox Wheel...but for tabletop games." />
        <meta property="og:image" content={`https://hooleymcknight.com/jackbox-wheel/jackbox-wheel.png`} />
        <meta name="og:title" content="Similar to the Jackbox Wheel...but for tabletop games." />
      </Head>

      <div className={styles['wheel-title']}>
        <h1>Tabletop Games Wheel</h1>
      </div>
      <WinnerGame className={styles['winner-title']} winningGame={winningGame} confettiAllowed={confettiAllowed} />
      <Wheel page='tabletop-games' styles={styles} activeGames={finalGameSet} setWinningGame={(e) => setWinningGame(e)} /> {/* make sure active games excludes disallowed games */}
      ...
      <TTGGamesList className={styles['games-list']}
        typeClassName={styles['pack-group']}
        gamesData={gamesData}
